...
class OctConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # High -> Low
        x_h2l = self.pool(x_h) if (self.out_channels['low'] > 0 and x_h.size(-1) > 1) else x_h
        x_h2l = self.conv_h2l(x_h2l) if self.out_channels['low'] > 0 else None

        if x_l is not None:
            # Low -> Low
            x_l2l = self.pool(x_l) if (self.out_channels['low'] > 0 and self.stride == 2 and x_l.size(-1) > 1) else x_l
